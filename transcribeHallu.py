...
# faster_whisper compute_type float16 int8_float16 int8
def loadModel(gpu: str, modelSize="medium", compute_type="int8"):
    global model
    global device
    global cudaIdx
    cudaIdx = gpu
    try:
        if whisperFound == "FSTR":
            ...
            if modelSize != 'tiny' and modelSize != 'base' and modelSize != "small" \
                    and modelSize != 'medium' and modelSize != 'large' and modelSize != "large-v2":
                # custom faster_whisper model
                modelSize = huggingface_hub.snapshot_download(
                    modelSize,
                    allow_patterns=[
                        "config.json",
                        "model.bin",
            ...
            model = WhisperModel(modelSize, device=device, device_index=int(gpu), compute_type=compute_type)
...
def transcribeOpts(path: str, opts: dict, lngInput=None, isMusic=False, addSRT=False):
    ...
    try:
        ...
        aCmd = "ffmpeg -y -i \"" + pathIn + "\" " + " -c:a pcm_s16le -ar " + str(
            SAMPLING_RATE) + " \"" + pathWAV + "\" > \"" + pathWAV + ".log\" 2>&1"
        print("CMD: " + aCmd)
        os.system(aCmd)
    ...
    if (useDemucs):
        ...
        # print("CMD: "+aCmd)
        # os.system(aCmd)
        demucs_audio(pathIn=pathIn, model=modelDemucs, device="cuda:" + cudaIdx, pathVocals=pathDemucs,
    ...
    try:
        ...
        aCmd = "ffmpeg -y -i \"" + pathIn + "\" -af \"silenceremove=start_periods=1:stop_periods=-1:start_threshold=-50dB:stop_threshold=-50dB:start_silence=0.2:stop_silence=0.2, loudnorm\" " + " -c:a pcm_s16le -ar " + str(
            SAMPLING_RATE) + " \"" + pathSILCUT + "\" > \"" + pathSILCUT + ".log\" 2>&1"
        print("CMD: " + aCmd)
        os.system(aCmd)
        print("T=", (time.time() - startTime))
    ...
    if (not isMusic and useSileroVAD):
        ...
        try:
            ...
            speech_timestamps = get_speech_timestamps(wav, modelVAD, threshold=0.5, min_silence_duration_ms=500,
...
def transcribeMARK(path: str, opts: dict, mode=1, lngInput=None, aLast=None, isMusic=False):
    ...
    if (mode == 0):
        ...
        try:
            if (mode != 3):
                ...
                aCmd = "ffmpeg -y -i " + mark1 + " -i \"" + pathIn + "\" -i " + mark2 + " -filter_complex \"[0:a][1:a][2:a]concat=n=3:v=0:a=1[a]\" -map \"[a]\" -c:a pcm_s16le -ar " + str(
                    SAMPLING_RATE) + " \"" + pathMRK + "\" > \"" + pathMRK + ".log\" 2>&1"
                print("CMD: " + aCmd)
                os.system(aCmd)
                print("T=", (time.time() - startTime))
            ...
            if (useCompressor and not isMusic):
                ...
                aCmd = "ffmpeg -y -i \"" + pathIn + "\" -af \"speechnorm=e=50:r=0.0005:l=1\" " + " -c:a pcm_s16le -ar " + str(
                    SAMPLING_RATE) + " \"" + pathCPS + "\" > \"" + pathCPS + ".log\" 2>&1"
                print("CMD: " + aCmd)
                os.system(aCmd)
                print("T=", (time.time() - startTime))
        ...
        if whisperFound == "FSTR":
            ...
            if (mode == 3):
                ...
                for segment in segments:
                    aSegCount += 1
                    result["text"] += "\n" + str(aSegCount) + "\n" + formatTimeStamp(
                        segment.start) + " --> " + formatTimeStamp(segment.end) + "\n" + segment.text.strip() + "\n"
            else:
                for segment in segments:
    ...
    if (mode == 2):
        ...
        if (
                re.match(r"^ *" + aOk + aSep + aWhisper + aSep + ".*" + aWhisper + aSep + aOk + aSep + " *$",
                         result["text"],
                         re.IGNORECASE)):
            # GOOD!
            result["text"] = aCleaned
